...
export var PaginationhalfComponent = (function () {
    function PaginationhalfComponent() {
        this.total = 0;
        this.page = 0;
        this.goTo = new EventEmitter();
        this.from = 1;
    }
    PaginationhalfComponent.prototype.totalPages = function () {
        return Math.ceil(this.total / 8);
    };
    PaginationhalfComponent.prototype.prevPage = function () {
        return Math.max(1, this.page - 9); // 페이지 이전으로 //10페이지씩
        // return Math.max(1, this.page ); // 페이지 이전으로
    };
    PaginationhalfComponent.prototype.pageFirst = function () {
        return Math.max(1, -1); // 페이지 처음으로 (0페이지 부터 시작 -> 화면은 1로 ...-1해야함)
    };
    PaginationhalfComponent.prototype.pageLast = function () {
        return Math.min(this.totalPages(), this.totalPages()); //페이지 마지막으로
    };
    PaginationhalfComponent.prototype.nextPage = function () {
        return Math.min(this.totalPages(), this.page + 11);
        // return Math.min(this.totalPages(),this.page + 2);
    };
    PaginationhalfComponent.prototype.pageClicked = function (page) {
        this.goTo.next(page);
    };
    PaginationhalfComponent.prototype.pagesRange = function () {
    ...
    PaginationhalfComponent.prototype.range = function (start, stop, step) {
    ...
    ], PaginationhalfComponent.prototype, "total", void 0);
    __decorate([
        Input(), 
        __metadata('design:type', Number)
    ], PaginationhalfComponent.prototype, "page", void 0);
    __decorate([
        Output(), 
        __metadata('design:type', EventEmitter)
    ], PaginationhalfComponent.prototype, "goTo", void 0);
    PaginationhalfComponent = __decorate([
        Component({
            moduleId: module.id,
            selector: 'paginationhalf',
